...
const VALID_PLAYLIST = {
    ...
    //songIds: Array(100).fill("spotify:track:5wv9OEeC3N0oWqLMATSoSK"),
}

const VALID_PLAYLIST_SONGIDS = {
    SK: "PLAYLIST#Playlist46C7xFmDg0YcpcJHOQpdjr",
    playlistName: "Gym going playlist",
    PK: "USER#user5dbd0e3b-6718-46ca-9867-f2df7765fea5",
    description: "Generated by HeartSync API",
...
describe("Playlist Builder service layer", () => {
    beforeEach(() => {
        jest.clearAllMocks();
        playlistRepository.getPlaylistbyPlaylistId.mockClear();
    });
    ...
    describe("populatePlaylist function", () => {
        test("Successfully returns a list of 100 Spotify URIs", async () => {
            ...
            lastFMService.getTracksByGenre.mockReturnValue(arr);
            lastFMService.getTracksByArtist.mockReturnValue(arr);
            spotifyService.getTrackURI.mockReturnValue("SpotifyURI");
            const dummyUserId = "dummyId";
            // Act
            const spotifyURIs = await populatePlaylist("playlistId", "genre", "artistName", dummyUserId);
            ...
            expect(playlistRepository.addSongsToPlaylist).toHaveBeenCalledWith(
                dummyUserId,
                "playlistId",
                expect.arrayContaining(Array(100).fill("SpotifyURI"))
            )
        ...
        test("Successfully returns a list of 50 Spotify URIs based on genre", async () => {
            ...
            const dummyUserId = "dummyId";
            // Act
            const spotifyURIs = await populatePlaylist("playlistId", "genre", null, dummyUserId);
        ...
        test("Successfully returns a list of 50 Spotify URIs based on artist", async () => {
            ...
            const dummyUserId = "dummyId";
            // Act
            const spotifyURIs = await populatePlaylist("playlistId", null, "artist", dummyUserId);
            ...
            expect(playlistRepository.addSongsToPlaylist).toHaveBeenCalledWith(
                dummyUserId,
                "playlistId",
                expect.arrayContaining(Array(500).fill("SpotifyURI"))
            )
        });

        test("Successfully returns null when no genre/artist is given", async () => {
            // Arrange
            playlistRepository.getPlaylistbyPlaylistId.mockReturnValue(VALID_PLAYLIST);
            userService.getUserById.mockReturnValue(VALID_USER);
            lastFMService.getTracksByGenre.mockReturnValue(null);
            ...
            expect(logger.error).toHaveBeenCalledWith("Failed to validate playlist population");
        });
...
describe("getPlaylistByPlaylist ID Suite", () => {
    ...
    test("Successfully returns the correct playlist when given right ID", async () => {
        // Arrange
        playlistRepository.getPlaylistbyPlaylistId.mockReturnValue(VALID_PLAYLIST_SONGIDS);
        playlistRepository.getInfoOnTracks.mockReturnValue([
            {
            "Title": "BALD! REMIX",
            "Artist": "JPEGMAFIA",
            "Album": "EP!",
        ...
        expect(playlist.songIds).toBeUndefined();
        //expect(playlist.tracksInfo.length).toBe(1);
